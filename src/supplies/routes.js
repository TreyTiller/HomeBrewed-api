...
suppliesRouter
  ...
  .get((req, res, next) => {
    suppliesService
      .getById(req.app.get("db"), req.params.recipe_id)
      .then(supplies => {
  ...
  .post(requireAuth, bodyParser, (req, res, next) => {
    req.body.forEach(item => {
      ...
      suppliesService
        .insertsupplies(req.app.get("db"), newsupplies)
        .then(supplies => {
...
suppliesRouter
  ...
  .all((req, res, next) => {
    ...
    suppliesService
      .getById(req.app.get("db"), supplies_id)
      .then(supplies => {
        if (!supplies) {
  ...
  .delete((req, res, next) => {
    const { supplies_id } = req.params;
    suppliesService
      .deletesupplies(req.app.get("db"), req.params.recipe_id, supplies_id)
